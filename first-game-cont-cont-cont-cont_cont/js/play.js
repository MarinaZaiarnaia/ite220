var playState = {
	...
	update: function(){
		...
		if(this.nextEnemiesUp < game.time.now){
			var start = 4000, end = 1000, score = 100;
			var delay = Math.max(
				start - (start - end) * game.global.score / score, end);
			this.addEnemiesUp();
		this.nextEnemiesUp = game.time.now + delay;
		}
		if(this.nextEnemiesDown < game.time.now){
			var start = 4000, end = 1000, score = 100;
			var delay = Math.max(
				start - (start - end) * game.global.score / score, end);
			this.addEnemiesDown();
		this.nextEnemiesDown = game.time.now + delay;
		}	
	},
